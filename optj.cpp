...
val_t parse_one(int end){
  ...
  case '0':
    if(pe[0]=='0'&&pe[1]=='1'){
      ...
      ++celln;
      cells[celln].car=S;
      cells[celln].cdr=mk_cons(celln-1);
      ++celln;
      while(*pe=='1'){
	++pe;
	cells[celln].car=K;
	...
	DIE();
      ...
      return mk_cons(celln-1);
    }
    if(pe[0]=='0'&&pe[1]=='0'&&pe[2]=='1'){
      pe+=3;
      // jot 0001(1..) -> (S`K`..)S`KS
      cells[celln].car=K;
      cells[celln].cdr=S;
...
bool is_jot0(val_t v){
  return is_ki(v)||is_cons(v)&&carof(v)==K&&is_jot0(cdrof(v));
}

bool is_jot001(val_t v){
  return is_cons(v)&&carof(v)==S&&is_cons(cdrof(v))&&carof(cdrof(v))==K&&(cdrof(cdrof(v))==K||is_jot001(cdrof(cdrof(v))));
}

bool is_jot0001(val_t v){
  return is_cons(v)&&carof(v)==S&&is_cons(cdrof(v))&&carof(cdrof(v))==K&&(cdrof(cdrof(v))==S||is_jot0001(cdrof(cdrof(v))));
}

bool is_jot(val_t v){
  return is_ki(v)||is_jot001(v)||is_jot0001(v);
}

std::string strfy_jot0(val_t v){
  return !is_cons(cdrof(v)) ? "0" : strfy_jot0(cdrof(v))+"1";
}

std::string strfy_jot001(val_t v){
  return !is_cons(cdrof(cdrof(v))) ? "001" : strfy_jot001(cdrof(cdrof(v)))+"1";
}

std::string strfy_jot0001(val_t v){
  return !is_cons(cdrof(cdrof(v))) ? "0001" : strfy_jot0001(cdrof(cdrof(v)))+"1";
}

//int lambdalevel;
std::string strfy(val_t v, bool need_paren=false,bool no_jot=false) {
  std::string s;
  if(is_cons(v)){
    bool car_jot=false;
    if(!no_jot && is_ki(v)){
      s+="0";
    }else if(!no_jot && is_jot0(v)){
      s+=strfy_jot0(v);
    }else if(!no_jot && is_jot001(v)){
      s+=strfy_jot001(v);
    }else if(!no_jot && is_jot0001(v)){
      s+=strfy_jot0001(v);
      ...
      if(need_paren){
        if(1){
          if(carof(v)==I) DIE();
          if(is_cons(carof(v))&&!is_jot(carof(v))){
      ...
      s+=strfy(cdrof(v),true,(s[s.size()-1]|1)=='1');
      s+=closing;
    }
  }else if(is_lambda(v)){
    DIE();
...
int main(int argc, char**argv){
  for(int i=1;i<argc;++i){
    if(memcmp(argv[i],"-v",2)==0){
      verbose=argv[i][2]?argv[i][2]-48:1;
    }
